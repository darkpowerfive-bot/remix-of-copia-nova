...
const logStep = (step: string, details?: any) => {
  ...
  console.log(`[SEND-PENDING-EMAIL] ${step}${detailsStr}`);
};
...
serve(async (req) => {
  ...
  try {
    ...
    const { email, fullName, whatsapp } = await req.json();
    ...
    let resolvedWhatsapp: string | undefined = whatsapp;
    ...
    if (!resolvedWhatsapp) {
      ...
      resolvedWhatsapp = (profileData?.whatsapp as string | null) ?? undefined;
    }

    // Send WhatsApp message if we have a number
    if (resolvedWhatsapp) {
      try {
        logStep("Sending WhatsApp pending message", { whatsapp: resolvedWhatsapp });
